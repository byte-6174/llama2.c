...
void transformer(int token, int pos, Config* p, RunState* s, TransformerWeights* w) {
    ...
    for(int l = 0; l < p->n_layers; l++) {
        ...
        //matmulint(s->q, s->xb, w->wq + l*dim*dim, dim, dim);
       
        matmul(s->k, s->xb, w->wk + l*dim*kv_dim, dim, kv_dim);
        //matmulint(s->k, s->xb, w->wk + l*dim*kv_dim, dim, kv_dim);

        matmul(s->v, s->xb, w->wv + l*dim*kv_dim, dim, kv_dim);
        //matmulint(s->v, s->xb, w->wv + l*dim*kv_dim, dim, kv_dim);
        ...
        matmulint(s->xb2, s->xb, w->wo + l*dim*dim, dim, dim);
    ...
    //matmulint(s->logits, x, w->wcls, p->dim, p->vocab_size);
}
